      ...
      if (Capacitor?.isNativePlatform?.()) {
        ...
        return await new Promise((resolve, reject) => {
          ...
          navigator.geolocation.getCurrentPosition(
            ...
            (error) => {
              console.warn(
                "[SplashScreen] 위치 정보 가져오기 실패, 3초 후 재시도...",
                error
              );
...
export default function SplashScreen({
    ...
    try {
      ...
      if (Capacitor?.isNativePlatform?.()) {
        ...
        if (status.location !== "granted") {
          ...
          throw new Error(
            "Geolocation denied: 앱 사용을 위해 위치 권한이 필수입니다. 설정에서 허용해주세요."
          );
        }
      }
      ...
      if (hasPharmacies && lastLocation) {
        ...
        if (latDiff >= tolerance || lngDiff >= tolerance) {
          ...
          console.log(
            "[SplashScreen] 이전 위치와 동일, 약국 데이터 재요청 건너뜀"
          );
        }
  ...
  const isLocationError =
    error?.includes("위치 권한이") || error?.includes("Geolocation denied");
  ...
  const handleGoToSettings = () => {
    if (Capacitor?.isNativePlatform?.()) {
      ...
      if ((App as any).openAppSettings) {
        ...
        alert(
          "자동 설정 이동 기능이 지원되지 않아 직접 설정으로 이동해주세요."
        );
      }
    } else {
      alert(
        "웹 환경에서는 설정 이동을 지원하지 않습니다. 브라우저 설정을 확인해주세요."
      );
    }
  };
