...
const PermissionModal = ({
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
    <div className="bg-white rounded-lg p-6 max-w-sm w-full">
      ...
      <p className="mb-6">
        약국 검색을 위해 위치 정보 접근 권한이 필요합니다. 설정에서 위치 권한을
        허용해주세요.
      </p>
...
const EmptyState = () => (
  <div className="text-center p-6">
    ...
    <p className="text-gray-500 text-sm">
      현재 위치에서 가까운 약국을 찾을 수 없습니다. 다른 위치에서
      시도해보시거나, 검색 범위를 넓혀보세요.
    </p>
  </div>
);
...
export default function PharmacyList({ onLocationLoaded }: PharmacyListProps) {
  ...
  const { pharmacies, isLoading, error, findNearbyPharmacies } =
    usePharmacies();
  const { requestLocationPermission, permissionStatus, checkPermission } =
  ...
  const getPharmacyData = (pharmacy: PharmacyWithUser): PharmacyData => {
    ...
    const id =
      (pharmacy as any).p_id ||
      (pharmacy as any).hpid ||
      `pharmacy-${Math.random().toString(36).substr(2, 9)}`;
    const name =
      (pharmacy as any).name || (pharmacy as any).yadmNm || "이름 없는 약국";
    const address = (pharmacy as any).addr || (pharmacy as any).address || "";
    const tel = (pharmacy as any).tel || "";
    const distance =
      typeof (pharmacy as any).distance === "number"
