...
>("pharmacy/fetchNearby", async ({ lat, lng, radius = 8000 }, { rejectWithValue }) => {
  try {
    ...
    const pharmaciesWithUser = await Promise.all(
      pharmacyList.map(async (pharmacy) => {
        try {
          ...
          console.error(`Error fetching user data for pharmacy ${pharmacy.p_id}:`, error);
          return { 
            ...pharmacy,
            latitude: pharmacy.latitude || pharmacy.lat || 0,
            longitude: pharmacy.longitude || pharmacy.lng || 0,
    ...
    return rejectWithValue(
      err.message || "약국 정보를 불러오는 데 실패했습니다."
    );
  }
});
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase("persist/REHYDRATE", (state, action: any) => {
        ...
        if (!state.isLoading && action.payload?.pharmacy) {
